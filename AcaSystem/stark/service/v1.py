...
def get_m2m_text(title, field):
    def inner(self, obj=None, is_header=False, *args, **kwargs):
        if is_header:
            return title
        query_set = getattr(obj, field).all()
        obj_str = ','.join([str(object) for object in query_set])
        return obj_str
    return inner
...
class StarkHandler(object):
    ...
    def display_edit_del(self, obj=None, is_header=None, *args, **kwargs):
        if is_header:
            return "操作"
        return mark_safe('%s %s' % (self.display_edit(obj, is_header), self.display_del(obj, is_header)))
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        queryset = self.get_queryset(request, *args, **kwargs).filter(conn).filter(**search_group_condition).order_by(*order_list)
    ...
    def add_view(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            self.save(form, False, request, *args, **kwargs)
    ...
    def change_view(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            self.save(form, True, request, *args, **kwargs)
            # 在数据库保存成功后，跳转回列表页面(携带原来的参数)。
            return redirect(self.reverse_list_url(*args, **kwargs))
        return render(request, 'stark/change.html', {'form': form})
