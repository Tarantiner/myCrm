...
from academy.models import ConsultRecord, Customer
...
class ConsultRecordHandler(PermissionHandler, StarkHandler):
    ...
    def get_change_object(self, request, pk, *args, **kwargs):
        customer_id = kwargs.get('customer_id')
        consult_id = 18
        return ConsultRecord.objects.filter(pk=pk, customer_id=customer_id,
                                                   customer__consultant_id=consult_id).first()

    def delete_object(self, request, pk, *args, **kwargs):
        customer_id = kwargs.get('customer_id')
        consult_id = request.session['user_info']['id']

        record_queryset = ConsultRecord.objects.filter(pk=pk, customer_id=customer_id,
                                                              customer__consultant_id=consult_id)
